...
def handle_pedido(messages, data, telefono, session_context, supabase_client=supabase_class):
    ...
    if "eliminar_platillo" in intencion:
        raw = llamar_llm(PROMPT_EXTRAER_EDICION(menu_data), messages[-4:], data["body"])
        try:
        ...
        raw = llamar_llm(PROMPT_EXTRAER_MODIFICACION(menu_data), messages[-4:], data["body"])
        try:
            raw = raw.strip().replace("```json", "").replace("```", "").strip()
        ...
        raw = llamar_llm(PROMPT_EXTRAER_ORDEN(menu_data), messages[-4:], data["body"])
        try:
            raw = raw.strip().replace("```json", "").replace("```", "").strip()
